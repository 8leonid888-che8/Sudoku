...
class Board:
    def __init__(self, n=3):
        ...
        self.keys_dict = list(self.missing_nums.keys())
    ...
    def press_cell(self, pos):
        if pos[2] == "big":
            if not self.board[pos[1]][pos[0]]:
                if self.last_selected_cell:
                    self.pressed_cell[self.last_selected_cell[1]][self.last_selected_cell[0]] = False
                self.pressed_cell[pos[1]][pos[0]] = True
        ...
        if pos[2] == "small":
            if self.moment_choice:
                num = self.keys_dict[pos[0]]
                if self.missing_nums[num] != 0:
                    self.board[self.last_selected_cell[1]][self.last_selected_cell[0]] = num
                    self.game_board[self.last_selected_cell[1]][self.last_selected_cell[0]] = num
    ...
    def render(self, screen):
        ...
        for h in range(2):
            for w in range(self.n ** 2):
                if h == 0:
                    font = pygame.font.Font(None, 35)
                    text = font.render(str(self.keys_dict[w]), True, Color("blue"))
                    screen.blit(text, (x + 12, y + 8))
                    pygame.draw.rect(screen, pygame.Color("white"), (x, y, self.cell_size, self.cell_size), 1)
                else:
                    font = pygame.font.Font(None, 35)
                    text = font.render(str(self.missing_nums[self.keys_dict[w]]), True, Color("blue"))
                    screen.blit(text, (x + 12, y + 8))
                    pygame.draw.rect(screen, pygame.Color("white"), (x, y, self.cell_size, self.cell_size), 1)
